...
if ROLE == "student":
    ...
    if "active_word" not in st.session_state or st.session_state.get("active_lid") != lid:
    ...
    if submit:
        ...
        with st.expander("Why are these the best choices?", expanded=True):
            lines = []
            for opt in choices:
                if opt in correct_set and opt in picked_set:
                    tag = "✅ correct (you picked)"
                elif opt in correct_set:
                    tag = "✅ correct"
                elif opt in picked_set:
                    tag = "❌ your pick"
                else:
                    tag = ""
