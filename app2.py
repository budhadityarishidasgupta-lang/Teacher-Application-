...
# LLM config (Gemini default, OpenAI fallback)
ENABLE_GPT     = os.getenv("ENABLE_GPT", "0") in ("1","true","yes","on")
LLM_PROVIDER   = os.getenv("LLM_PROVIDER", "gemini").strip().lower()
SHOW_EXAMPLES  = os.getenv("SHOW_EXAMPLES", "0") in ("1","true","yes","on")
...
gem_client = None
if ENABLE_GPT and LLM_PROVIDER == "gemini" and GEMINI_API_KEY:
    try:
        import google.generativeai as genai
        genai.configure(api_key=GEMINI_API_KEY)
        gem_client = genai.GenerativeModel(GEMINI_MODEL)
    except Exception:
        gem_client = None
...
def patch_courses_table():
    """Ensure legacy courses/lessons/words have required columns and indexes."""
    with engine.begin() as conn:
        ...
        # Ensure unique headword to enable upsert on words
        conn.execute(text(
            "DO $$ BEGIN "
            "IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname='ux_words_headword') THEN "
            "CREATE UNIQUE INDEX ux_words_headword ON words (headword); "
            "END IF; END $$;"
        ))
...
# ─────────────────────────────────────────────────────────────────────
# Option reuse control (avoid repeating any option within last N Qs)
# ─────────────────────────────────────────────────────────────────────
OPTION_COOLDOWN = 8  # do not repeat any option within the last 8 questions
if "recent_options" not in st.session_state:
    st.session_state.recent_options = []  # rolling window of recently displayed option strings

def _blocklisted_options():
    # Keep at most 6 options * last OPTION_COOLDOWN questions
    window = OPTION_COOLDOWN * 6
    return set(st.session_state.recent_options[-window:])

def _remember_options(options):
    st.session_state.recent_options.extend(list(options))
    window = OPTION_COOLDOWN * 6
    if len(st.session_state.recent_options) > window:
        st.session_state.recent_options = st.session_state.recent_options[-window:]
...
def mastered_count(user_id, lesson_id):
    ...
    q = text("""
        SELECT COUNT(*) AS c
        FROM word_stats
        WHERE user_id=:u AND mastered=TRUE AND headword IN :arr
    """).bindparams(bindparam("arr", expanding=True))

    m = pd.read_sql(q, con=engine, params={"u": int(user_id), "arr": list(set(words))})["c"].iloc[0]
...
def course_progress(user_id: int, course_id: int):
    ...
    completed = pd.read_sql(q, con=engine, params={"u": int(user_id), "arr": list(set(all_words))})["c"].iloc[0]
...
def build_question_payload(headword: str, synonyms_str: str):
    ...
    Build a 6-option question:
      ...
      - 4 distractors from a safe pool
      - Avoid reusing any option shown in the last OPTION_COOLDOWN questions
    """
    ...
    # --- avoid reusing options shown recently ---
    block = _blocklisted_options()
    lower_correct = {c.lower() for c in correct}
    pool = [d for d in distractor_pool if (d.lower() not in lower_correct and d not in block)]
    ...
    distractors = []
    for cand in pool:
        if len(distractors) >= 4: break
        if cand not in distractors:
            distractors.append(cand)

    # If not enough fresh distractors, allow older ones (still not correct)
    if len(distractors) < 4:
        refill = [d for d in distractor_pool if d.lower() not in lower_correct and d not in distractors]
        rnd.shuffle(refill)
        for cand in refill:
            if len(distractors) >= 4: break
            distractors.append(cand)

    choices = (correct + distractors)[:6]
    rnd.shuffle(choices)
    return {"headword": headword, "choices": choices, "correct": set(correct)}

# ─────────────────────────────────────────────────────────────────────
# LLM helpers (Gemini preferred, OpenAI fallback). Examples are gated by SHOW_EXAMPLES.
# ─────────────────────────────────────────────────────────────────────
def gpt_feedback_examples(headword: str, correct_word: str):
    """
    Returns (why, [ex1, ex2]) with kid-friendly, spoken-English sentences.
    Uses OpenAI when enabled; otherwise a simple fallback.
    """
    def _fallback():
...
Rules:
- "why": 1 short sentence (≤ 16 words) explaining why "{correct_word}" matches "{headword}".
- "examples": TWO different sentences commonly used by English speakers; perfect grammar; ages 7–11.
- Use "{correct_word}" exactly once per example; avoid using "{headword}" unless natural.
- 8–12 words each. No quotes; no semicolons; no rare words.
- Positive, clear tone. Return valid JSON only.
"""
...
def gemini_feedback_examples(headword: str, correct_word: str):
    """
    Returns (why, [ex1, ex2]) via Gemini; falls back to local examples on failure.
    """
    def _fallback():
        why = f"'{correct_word}' is a good synonym for '{headword}' because they mean almost the same thing."
        return why, [
    ...
    if not (ENABLE_GPT and gem_client):
        return _fallback()
...
Return JSON only: {{"why": "...", "examples": ["...", "..."]}}

Rules:
- "why": 1 short sentence (≤ 16 words) explaining why "{correct_word}" matches "{headword}".
- "examples": TWO sentences commonly used by English speakers; perfect grammar; ages 7–11.
- Use "{correct_word}" exactly once per example; avoid also using "{headword}" unless natural.
- 8–12 words each; no quotes; no rare words; positive tone.
"""
...
if ROLE == "student":
    ...
    if new_word_needed:
        ...
        _remember_options(st.session_state.qdata["choices"])
    ...
    # The quiz form (Submit only; Next lives outside after feedback)
...
if ROLE == "student" and st.session_state.get("answered") and st.session_state.get("eval"):
    ...
    # LLM: brief why (+ examples hidden unless SHOW_EXAMPLES=1)
    try:
        correct_choice_for_text = sorted(list(ev["correct_set"]))[0]
        if LLM_PROVIDER == "gemini":
            why, examples = gemini_feedback_examples(st.session_state.active_word, correct_choice_for_text)
        else:
